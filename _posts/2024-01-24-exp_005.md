...
- ✨ 챗봇의 5가지 대표 유형(그림출처 : [Tony Around](https://tonyaround.com/%ec%b1%97%eb%b4%87-%ea%b8%b0%ed%9a%8d-%eb%8b%a8%ea%b3%84-%ec%b1%97%eb%b4%87%ec%9d%98-5%ea%b0%80%ec%a7%80-%eb%8c%80%ed%91%9c-%ec%9c%a0%ed%98%95-%ec%a2%85%eb%a5%98/))
	- 대화형 챗봇 : 머신러닝/딥러닝 기반 자연어 처리
		![img1](https://i0.wp.com/tonyaround.com/wp-content/uploads/2019/09/iconfinder_customer_live_chat_service_4104737-1.png?w=512&ssl=1)	
	- 트리형(버튼) 챗봇 : 정해진 트리에 따라 답변
		![img3](https://i0.wp.com/tonyaround.com/wp-content/uploads/2019/09/unnamed-file.png?resize=1024%2C533&ssl=1)
	- 추천형 챗봇 : 사전 정의된 답변를 알고리즘을 통해 우선 순위별로 리스트
		![img2](https://i0.wp.com/tonyaround.com/wp-content/uploads/2019/09/unnamed-file-2.png?w=695&ssl=1)
	- 시나리오형 챗봇 : 원하는 서비스 제공을 위해 정해진 시나리오를 수행
		![img4](https://i0.wp.com/tonyaround.com/wp-content/uploads/2019/09/unnamed-file-3.png?w=893&ssl=1)
	- 결합형 챗봇 : 비즈니스 목적에 따라 여러 챗봇 유형들을 겹합
		![img5](https://i0.wp.com/tonyaround.com/wp-content/uploads/2019/09/unnamed-file-4.png?resize=1024%2C557&ssl=1)
...
- `기존 자연어 처리와의 차이점 : 포지셔널 인코딩(positional Encoding)`
	...
	- 수식
		![img11](https://d3s0tskafalll9.cloudfront.net/media/original_images/Untitled_7_3Rneu0P.png)
...
- 어텐션 값이란? `단어간의 유사도(단어간의 거리|유클리드)`
	...
	- 어텐션 값 도출 과정
		...
		- 📌주요 용어
			- 쿼리(Query) : 기준 단어 임베딩(pos x d_model) X $W_{Query}$
			- 키(Key) : 측정할 단어 임베딩(pos x d_model) X $W_{Key}$
			- 값(Value) : 측정할 단어 임베딩(pos x d_model) X $W_{Value}$
...
- 트랜스포머에 사용된 어텐션
	...
	- 📌예시 : 저는 학생입니다. → I am a student
		- 1번 어텐션 : `저는 학생입니다.`라는 문장 내 단어들 간의 유사도 측정
			- 목적 : 한국어의 어순 데이터?
		- 2번 어텐션 : `I am student`라는 문장 내 단어들 간의 유사도 측정
			- 목적 : 영어의 어순 데이터?
